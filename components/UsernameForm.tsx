...
export function UsernameForm({ hints }: UsernameFormProps) {
  ...
  // Auto-focus on desktop only. On mobile this pops the keyboard before the
  // user can read the hero copy.
  useEffect(() => {
  ...
  return (
    <div className="flex w-full max-w-sm flex-col items-center gap-3">
      ...
      {hints && hints.length > 0 && (
        <p className="flex flex-wrap items-center justify-center gap-x-1 text-xs text-gray-400">
          <span>Try</span>
          {hints.map((hint, i) => (
            <span key={hint} className="flex items-center gap-x-1">
              <button
                ...
                className="min-h-[44px] px-1 text-gray-300 underline decoration-gray-600 underline-offset-2 transition-colors hover:text-green-400"
              >
                {hint}
              </button>
              {i < hints.length - 1 && <span className="text-gray-500">·</span>}
            </span>
          ))}
        </p>
      )}
